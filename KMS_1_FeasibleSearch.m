...
if ~isempty(theta_fminimax)
    if parallel & BCS_EAM ~= 1
        ...
        for ll = 1:num_fminimax
            [~,CV_fminimax,theta_feas,CV_maxviol] = KMS_31_Estep(theta_fminimax(ll,:),f_ineq,f_eq,f_ineq_keep,f_eq_keep,f_stdev_ineq,f_stdev_eq,G_ineq,G_eq,KMSoptions);
            if CV_maxviol <= CVtol
                flag_feas = 1;
                break;
            end
