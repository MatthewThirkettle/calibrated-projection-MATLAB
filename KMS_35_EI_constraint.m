...
if BCS_EAM == 1
   labmda = theta(component);
    if nargout <= 2
    c_theta    = predictor(labmda,dmodel);
    else
    [c_theta,dc_theta] = predictor(labmda,dmodel);
    end
else   
    if nargout <= 2
...
if nargout > 2
	...
	if BCS_EAM == 1
	  	term3 = -((Dh_theta - repmat(dc_theta.*q',[J,1]))*s - repmat(h_theta-c_theta,[1,dim_p]).*repmat(ds.*q',[J,1]))/s^2;
	else
