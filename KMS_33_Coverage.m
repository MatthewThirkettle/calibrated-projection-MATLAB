...
S           = KMSoptions.S;
boundary    = KMSoptions.boundary;
LB_theta    = KMSoptions.LB_theta;
UB_theta    = KMSoptions.UB_theta;
n           = KMSoptions.n;
resid_tol   = KMSoptions.CVX_resid_tol;
...
A = zeros(J + 4*dim_p + 2 + S,dim_p);
b = zeros(J + 4*dim_p + 2 + S,1);
...
% Next 2*dim_p entries of (A,b) requie that lambda is in the local
% parameter space sqrt(n)*(Theta-theta).
if boundary == 1
A(J + 2*dim_p + 1:J + 3*dim_p,:) = eye(dim_p,dim_p);
A(J + 3*dim_p + 1:J + 4*dim_p,:) = -eye(dim_p,dim_p);
b(J + 2*dim_p + 1:J + 3*dim_p,1) = sqrt(n)*(UB_theta - theta);
b(J + 3*dim_p + 1:J + 4*dim_p,1) = -sqrt(n)*(LB_theta - theta);
end

% Add in rho polytope constraints
A(J + 4*dim_p + 2 + 1 : J + 4*dim_p + 2 + S, :) = A_rho;
b(J + 4*dim_p + 2 + 1 : J + 4*dim_p + 2 + S, 1) = b_rho;
