function [KMSoptions] = KMSoptions_BCS_Simulation()
...
KMSoptions.B            = 301; % Bootstrap repitions                       (Smaller number = faster, but more sensitive.  Recommended set B in [2000,5000].
...
KMSoptions.EI_multi_num = 10;   % Maximum number of initial points to use in multi start.  (Smaller = faster, but more sensitive.  Recommend values in [10,100]).
...
KMSoptions.numgrad      = false;             % Set equal to true to compute Dg using numerical gradients. 
...
KMSoptions.EAM_tol = 0.005;                    % Require ||thetaU(iter) -thetaU(iter-1)||<EAM_tol
...
KMSoptions.unif_num         = 0;                      % Number of points to uniformly draw to add to list on each iteration
