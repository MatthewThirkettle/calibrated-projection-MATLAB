...
for iter=1:EAM_maxit
   ...
   if isempty(I) ==0
    ...
    if sgn_q == 1
        ...
        theta_eps3 = min(theta_hash.' + q.'*delta3,UB_theta.');  
    else
        theta_eps1 = max(theta_hash.' + q.'*delta1,LB_theta.'); 
        theta_eps2 = max(theta_hash.' + q.'*delta2,LB_theta.');
        theta_eps3 = max(theta_hash.' + q.'*delta3,LB_theta.'); 
    end
    theta_eps = [theta_eps1;theta_eps2;theta_eps3];
    ...
    if abs(opt_val - opt_bound) < 1e-4
        ...
        feas = find(maxviol_Astep <= CVtol);
        theta_feas_out = theta_Astep(feas,:);
        return;
    end
